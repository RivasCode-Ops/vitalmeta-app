...
export function registerOAuthRoutes(app: Express) {
  app.get("/api/oauth/callback", async (req: Request, res: Response) => {
    ...
    try {
      ...
      res.cookie(COOKIE_NAME, accessToken, { ...cookieOptions, maxAge: ACCESS_TOKEN_EXPIRY_MS });

      // Issue refresh token (rotativo com jti)
      const user = await db.getUserByOpenId(userInfo.openId);
      if (user) {
        const refreshToken = await sdk.createRefreshToken(user.id);
